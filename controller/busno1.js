app.controller('busno1',['$scope','$timeout','$http','$rootScope','$state','$localStorage',function($scope,$timeout,$http,$rootScope,$state,$localStorage){
    $scope.flag = false;
    $timeout(
       function(){
        $scope.latmaps();
    ...
    $scope.latmaps = function(){
        $.getJSON("https://api.thingspeak.com/channels/471817/fields/1/last.json?api_key=15NOU6F5VZS16YUW", function(result){
            $scope.m = result;
   ...
   $scope.lngmaps = function(){
         $.getJSON("https://api.thingspeak.com/channels/471817/fields/2/last.json?api_key=15NOU6F5VZS16YUW", function(result){ 
        $scope.n = result;
